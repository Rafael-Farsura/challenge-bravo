...
export class CurrencyService {
  ...
  private async getValueInUSD(currencyCode: string): Promise<number> {
    ...
    if (!currency) {
      const realCurrencyInUSD = await this.fetchExchangeRate(currencyCode);
  ...
  private async isCurrencySupported(
    ...
    return Promise.resolve(
      (await this.findOneCurrency(currencyCode, isAddMethod)) ? true : false,
    );
  }
  ...
  async findOneCurrency(
    ...
    if (!currency && !isAddMethod)
      throw new NotFoundException(`Could not find currency : ${currencyCode}`);
